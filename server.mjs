...
function buildCvPrompt(d) {
  const f = (v) => (v || "").toString().trim();
  return [
    "You are a top UK CV writer and recruiter. Produce a FLAWLESS, GOLD-STANDARD, ATS-friendly CV for an ENTRY-LEVEL job seeker",
    "targeting roles like retail, hospitality, customer service, admin, care, warehouse and apprenticeships in Greater Manchester.",
    "The CV must be so well-structured and well-worded that no recruiter could fault it.",
    "",
    "Rules (follow all):",
    "- Write a sharp, confident professional summary (2–3 sentences) tailored to the target role, leading with their strongest selling points and weaving in 2–3 keywords an ATS/recruiter would scan for.",
    "- Turn the work history into concise, achievement-focused bullets that START WITH STRONG ACTION VERBS (Served, Handled, Trained, Resolved, Operated…) and quantify wherever plausible (numbers, frequency, scale).",
    "- Treat part-time, weekend, volunteering, school and informal experience as real experience and frame it professionally. NEVER invent employers, qualifications, grades or dates — use only what is given; if a date is vague, keep it vague.",
    "- Produce a tight, relevant skills list mixing soft skills (teamwork, communication, reliability, time-keeping) with any practical/technical ones mentioned.",
    "- Reflect real education with grades/dates if provided. Fold certifications/licences, availability and right-to-work into the additional section.",
    "- UK English, concise, consistent tense, no clichés, no first-person pronouns, no unexplained gaps where information exists.",
    "",
    "APPLICANT DETAILS:",
    `Name: ${f(d.name) || "the applicant"}`,
    f(d.targetRole) ? `Target role(s): ${f(d.targetRole)}` : "Target: entry-level roles in Greater Manchester",
    f(d.experience) ? `Work history: ${f(d.experience)}` : "Work history: little/no formal experience — emphasise attitude, reliability and transferable skills.",
    f(d.education) ? `Education: ${f(d.education)}` : "",
    f(d.skills) ? `Skills/strengths: ${f(d.skills)}` : "",
    f(d.certs) ? `Certifications/licences: ${f(d.certs)}` : "",
    f(d.licence) ? `Driving licence: ${f(d.licence)}` : "",
    f(d.rtw) ? `Right to work in UK: ${f(d.rtw)}` : "",
    f(d.availability) ? `Availability: ${f(d.availability)}` : "",
    `References: ${f(d.references) || "available on request"}`,
    "",
    ...
    "headline = a short professional title line (e.g. 'Reliable Retail & Customer Service Assistant'). Put certifications, availability, right-to-work and 'References available on request' into the additional array. Fill every section as fully as the input allows.",
  ].filter(Boolean).join("\n");
}
